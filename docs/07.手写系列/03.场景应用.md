...
## 给了一个树的数组给出一个目标 id找到它所有的父子节点的 id
...
function getAllRelatedIds(tree, targetId) {
  const parentIds = findPathToNode(tree, targetId) || [];
  const targetNode = findNodeById(tree, targetId);
  const childIds = targetNode ? collectDescendants(targetNode) : [];
...
console.log(getAllRelatedIds(tree, 2));
//{
//  parentIds: [1],
//  childIds: [4, 5]
//}
